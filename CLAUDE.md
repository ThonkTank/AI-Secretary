...
**Multi-domain assistant:** The assistant tab is a full Claude Messages chat, wired in `features/task/application/assistant/` (`AssistantChatUseCase` engine, `AssistantConversation` session singleton, `AssistantTools` catalogue, `AssistantProposals` value types, `ConfirmAssistantProposalUseCase`). It lives in the task feature because task has no domain repository; it reaches the meal and budget features only through their domain layers via gateways in `application/internal/{meal,budget}/` (`AssistantMealGateway`, `AssistantBudgetGateway`, `AssistantTransactionImportExecutor`). Read tools (`get_*`) answer questions from the live DB on the `dbExecutor`; write tools (`propose_*`) never write — they park a validated `PendingProposal` shown as a confirmation card, and only `ConfirmAssistantProposalUseCase` executes it on user tap (task changes keep their `ApplyTaskChangesUseCase` undo). Model (`ClaudeModelStore`), endpoint (`ClaudeEndpointStore`) and thinking (a per-model ADAPTIVE→ENABLED→NONE fallback ladder) are user-configurable in the chat; PDF/txt/md attachments are supported, and an attached bank statement can be imported through the same dedup/fingerprint pipeline as the budget file import (fingerprint kept in sync with `BudgetImportUseCase.buildTransactionFingerprint`).
