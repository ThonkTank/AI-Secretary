...
**Budget recurring templates:** `BudgetRecurringTemplateEntity` (table `budget_recurring_template`) stores payee/amount/schedule patterns detected by `RecurringPatternDetector`. `BudgetCategory` has `icon` and `colorHex` fields for display. `BudgetTransactionEntity` has `transactionKind` (`STANDARD` / `INTERNAL_TRANSFER`) and `linkedTransactionId` for transfer pairs. `TransferDetails` (`features/budget/domain/`) is a value record (sourceAccountId, targetAccountId, amountCents, bookingDate, note) used by `BudgetRepository.createTransfer()` and `updateTransfer()` to avoid long parameter lists.
