...
public class SuggestionScorerTest {
    ...
    public void calculateConfidence_capsAtOne() {
        List<RecurringBudgetTransaction> txList = List.of(
        ...
        DatePatternDetector.PatternResult pattern =
                new DatePatternDetector.PatternResult(RecurringBudgetTransaction.RecurringType.MONTHLY_DAY, 1, null);
    ...
    public void calculateConfidence_rewardsKnownSubscription() {
        List<RecurringBudgetTransaction> known = List.of(tx("Spotify AB"), tx("Spotify AB"), tx("Spotify AB"));
        List<RecurringBudgetTransaction> unknown = List.of(tx("Local Shop"), tx("Local Shop"), tx("Local Shop"));
        DatePatternDetector.PatternResult pattern =
                new DatePatternDetector.PatternResult(RecurringBudgetTransaction.RecurringType.MONTHLY_DAY, 1, null);
    ...
    public void calculateConfidence_penalizesLargeVariance() {
        List<RecurringBudgetTransaction> txList = List.of(tx("Service"), tx("Service"), tx("Service"));
        DatePatternDetector.PatternResult pattern =
                new DatePatternDetector.PatternResult(RecurringBudgetTransaction.RecurringType.INTERVAL, 30, null);
    ...
    private RecurringBudgetTransaction tx(String payee) {
        return new RecurringBudgetTransaction.Builder(10L, -1000, LocalDate.of(2025, 1, 1), 5L)
                .payee(payee)
                .build();
    }
}
