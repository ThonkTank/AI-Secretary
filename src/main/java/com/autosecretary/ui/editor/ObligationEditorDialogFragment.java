...
public final class ObligationEditorDialogFragment extends DialogFragment {
    ...
    private void installInputListeners() {
        ...
        binding.DeadlineToday.setOnClickListener(view -> setDeadline(LocalDate.now().toString()));
    ...
    private void selectDeadline(String value) {
        ...
        else if (date.equals(LocalDate.now())) id = R.id.DeadlineToday;
        else if (date.equals(LocalDate.now().plusDays(1))) id = R.id.DeadlineTomorrow;
    ...
    private static String inference(ObligationEditorState state) {
        LocalDate date = parseDate(state.deadlineInput());
        if (date == null) return "Wird vorgeschlagen, sobald Platz ist.";
        if (date.equals(LocalDate.now())) return "Wird für heute vorgeschlagen, sobald Platz ist.";
        if (date.equals(LocalDate.now().plusDays(1))) return "Wird für morgen vorgeschlagen.";
