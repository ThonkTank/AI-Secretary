...
| Class | Role |
|---|---|
| `BudgetFragment` | Main budget screen fragment. Inflates views, observes `BudgetViewModel` LiveData, owns the `OpenDocument` launcher for statement import, and delegates dialog operations to `*DialogController` objects. Entry point from `MainActivity`. |
| `BudgetViewModel` | Central state owner. Loads data via the repository + `LoadBudgetOverviewUseCase`, owns account/filter selection, exposes 11 LiveData streams. All DB work runs on the injected `ExecutorService`. |
...
BudgetViewModel (ViewModel)
  └─ BudgetRepository / LoadBudgetOverviewUseCase  ← background executor
