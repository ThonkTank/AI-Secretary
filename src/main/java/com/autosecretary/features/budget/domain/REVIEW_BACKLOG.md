...
- [consider] `RecurringBudgetTransaction.java` — mutable public-field POJO with a factory method, inconsistent with all other domain DTOs in this package (`RecurringSuggestion`, `RecurringScheduleParams`, `TemplateStatusUpdate`, `CategorySpendSummary`, etc.) which are immutable records. Room's field-injection pattern in the data layer requires the mutable POJO shape, but this class is a domain type — the data layer mapper should produce and consume its own entity/projection, mapping to an immutable domain record. **Deferred reason:** Fixing requires decoupling the data-layer mapping from the domain shape — similar scope to the `BudgetRepository` / data-entity coupling issue above.
