...
/**
 * Global scheduling buffer configuration, resolved once per generation run.
 *
 * <p>Both values shape the placement math only — persisted slot times stay the real
 ...
 */
public record SchedulingTuning(int slotPauseMinutes, int appointmentLeadMinutes) {

    /** No buffers — plain back-to-back placement (pre-buffer behaviour). */
    public static final SchedulingTuning NONE = new SchedulingTuning(0, 0);
}
