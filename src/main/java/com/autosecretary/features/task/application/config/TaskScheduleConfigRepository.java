...
public class TaskScheduleConfigRepository implements SchedulingWindowProvider {
    public record ScheduleWindowRow(
            DayOfWeek dayOfWeek,
            LocalTime startTime,
            LocalTime endTime) {
    }
    ...
    public List<ScheduleWindowRow> loadAllRows() {
        return loadAll().stream()
                .map(config -> new ScheduleWindowRow(config.dayOfWeek, config.startTime, config.endTime))
                .toList();
    }
    ...
    public void saveAllRows(List<ScheduleWindowRow> rows) {
        List<TaskScheduleConfig> configs = rows.stream()
                .map(row -> new TaskScheduleConfig(row.dayOfWeek(), row.startTime(), row.endTime()))
                .toList();
        saveAll(configs);
    }
