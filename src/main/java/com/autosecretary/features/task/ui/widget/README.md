...
2. **TaskWidgetService** (`RemoteViewsService`)
   ...
   - Obtains TaskDao from AppCompositionRoot

3. **TaskWidgetFactory** (`RemoteViewsFactory`)
   - Loads task data from the database
   - Filters tasks to those scheduled on the selected date
...
AppWidgetManager calls TaskWidgetService.onGetViewFactory()
  → TaskWidgetFactory loads tasks and builds list of RemoteViews
```
...
2. **No date-scoped database query:**
   - Currently, the widget fetches **all** tasks from the database and filters them in memory
