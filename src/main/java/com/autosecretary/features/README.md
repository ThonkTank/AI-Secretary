...
- **Task → Meal:** Tasks can reference planned meals via `TaskPlannedMeal`. The task-owned `TaskMealCompletionFromMealPlanner` adapter in `task/application/internal/meal/` depends directly on the meal-domain repositories and handles pantry depletion and consumption logging on completion.
...
- [CLAUDE.md](../../../../../CLAUDE.md) — glossary, conventions, and project notes
