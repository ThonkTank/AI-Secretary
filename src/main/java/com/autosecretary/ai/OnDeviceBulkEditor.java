...
public final class OnDeviceBulkEditor implements AiProposalGateway {
    ...
    public Future<?> installBundledModel(Runnable onSuccess, Consumer<Throwable> onError) {
    ...
    public Future<?> propose(
        ...
        return aiExecutor.submit(() -> {
            try {
                BulkChangeProposal compiled = commands.compile(instruction, current, clock.now());
                if (compiled != null) {
                ...
                BulkChangeProposal proposal = parser.parse(response, current, clock.now());
