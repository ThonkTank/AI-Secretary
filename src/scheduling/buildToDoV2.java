...
public class buildToDoV2 {
     ...
     *      
     *      für alle Slots in allSlots
     *          fillSlots(Slot.item, Slot, allSlots) //falls sich prioritäten der Tasks geändert haben
     *      
     ...
     *              allSlots.add(this)
     *      allSlots chronologisch sortieren
     * }
     * 
     * getItems(typ, parent) {
    ...
    public List<todoList> planWeek() {
        ...
        // 4. Für alle befüllten Slots: Tasks innerhalb der Goal-Slots platzieren
        //    (falls sich Prioritäten der Tasks geändert haben)
        for (SlotCandidate sc : new ArrayList<>(allSlots)) {
            if (sc.slot().item != null) {
                fillSlots(trackedItem.ItemType.TASK, sc.slot().item, allSlots);
            }
        }
    ...
    private Match tryMatch(List<PrioritizedItem> itemList, List<SlotCandidate> slotList) {
        for (PrioritizedItem pi : itemList) {
            ...
            for (SlotCandidate c : slotList) {
                ...
                // Slot-Abdeckung: wenn Slot kürzer als benötigte Zeit,
    ...
    private List<SlotCandidate> aggregateSlots(List<todoList> lists) {
        ...
        allSlots.sort((a, b) -> a.slot().start.compareTo(b.slot().start));
        return allSlots;
    }
    ...
    private void assignSlot(trackedItem item, SlotCandidate candidate, List<SlotCandidate> slotList) {
        ...
        if (item.type == trackedItem.ItemType.GOAL) {
            // Virtuelle todoList für Goal-Innenraum
            todoList virtual = new todoList();
            virtual.date = list.date;
            virtual.start = slot.start;
            virtual.end = slot.end;
            virtual.timeSlots = (slot.timeSlots != null)
                    ? new ArrayList<>(slot.timeSlots) : new ArrayList<>();

            List<SlotCandidate> taskSlots = aggregateSlots(List.of(virtual));
            fillSlots(trackedItem.ItemType.TASK, item.id, taskSlots);

            // Sub-Slots übernehmen
            slot.timeSlots = virtual.timeSlots;

            // slot.end auf Ende des tatsächlichen Inhalts setzen
            if (slot.timeSlots != null && !slot.timeSlots.isEmpty()) {
                LocalTime actualEnd = slot.start;
                for (TimeSlot ts : slot.timeSlots) {
