...
public class BuildToDo {
    ...
    public List<TodoList> planWeek() {
        ...
        cachedBaseBudget = controller.budgetTab.BudgetManager.calculateFreeBudget(repo, null, today);
        ...
        // 3. Goals + Orphan Tasks (ohne Parent) gemeinsam in freie Slots platzieren
        fillTopLevel(allSlots);
    ...
    private int getFreeBudgetCents(Long accountId, LocalDate today) {
        ...
        int baseBudget = (accountId == null && cachedBaseBudget >= 0)
            ? cachedBaseBudget
            : controller.budgetTab.BudgetManager.calculateFreeBudget(repo, accountId, today);
    ...
    // ============================================================================
    // fillTopLevel - Goals + Orphan Tasks (ohne Parent) gemeinsam verplanen.
    //   Beide konkurrieren direkt um Top-Level-Slots nach Priorität.
    // ============================================================================
    private void fillTopLevel(List<SlotCandidate> slotList) {
        while (true) {
            // Goals + Orphan Tasks gemeinsam holen
            List<TrackedItem> items = getItems(TrackedItem.ItemType.GOAL, null);
            List<TrackedItem> orphanTasks = getItems(TrackedItem.ItemType.TASK, null);
            orphanTasks.removeIf(item -> item.parent != null);
            items.addAll(orphanTasks);
