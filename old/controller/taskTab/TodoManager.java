...
public class TodoManager {
    ...
    public interface TodoListener {
        ...
        default void onSchedulingConflicts(List<BuildToDo.SchedulingConflict> conflicts) {}
    }
    ...
    public void replanToday() {
        ...
        if (listener != null) {
            ...
            List<BuildToDo.SchedulingConflict> conflicts = planner.getConflicts();
    ...
    public List<TaskEntry> provideList(LocalDate day) {
        ...
        // Rezepte fuer Meal-Items vorladen (vermeidet N+1 Queries)
        Map<Long, String> recipeTitles = new HashMap<>();
        for (TrackedItem mealItem : repo.fetchAll(Table.ITEMS)) {
            if (mealItem.mealType != null && mealItem.plannedMeals != null) {
                TrackedItem.PlannedMeal pm = mealItem.getPlannedMealForDate(day);
                if (pm != null && !recipeTitles.containsKey(pm.recipeId())) {
                    Recipe recipe = repo.fetch(Table.RECIPES, pm.recipeId());
                    if (recipe != null) recipeTitles.put(pm.recipeId(), recipe.title);
                }
            }
        }
        ...
        for (TodoList.TimeSlot goalSlot : currentDayList.timeSlots) {
            ...
            for (TodoList.TimeSlot taskSlot : goalSlot.timeSlots) {
                ...
                if (task.mealType != null) {
                    TrackedItem.PlannedMeal pm = task.getPlannedMealForDate(day);
                    if (pm != null) {
                        String recipeTitle = recipeTitles.get(pm.recipeId());
                        if (recipeTitle != null) {
                ...
                if (slotId.equals(taskSlot.id)) {
                    ...
                    if (itemForMeal != null && itemForMeal.mealType != null) {
                        try {
                            ...
                            mealMgr.completeMealItem(itemForMeal, java.time.LocalDate.now(), 0);
                        } catch (Exception e) {
