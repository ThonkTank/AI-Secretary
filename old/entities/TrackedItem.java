...
public class TrackedItem {
    ...
    public List<PlannedMeal> plannedMeals;  // Rezept-Zuweisungen pro Datum (null wenn kein Meal-Item)

    /** Geplante Mahlzeit an einem bestimmten Datum. */
    public record PlannedMeal(LocalDate date, long recipeId, int servings,
                              boolean completed, int actualServings) {
        /** Gibt eine completed-Kopie zurueck. */
        public PlannedMeal withCompleted(int actual) {
            return new PlannedMeal(date, recipeId, servings, true, actual > 0 ? actual : servings);
        }
    }
    ...
    public static class Builder {
        ...
        public Builder plannedMeals(List<PlannedMeal> v) { item.plannedMeals = v; return this; }
    ...
    public void preserveState(TrackedItem source) {
        ...
        this.plannedMeals = source.plannedMeals;
    }
    ...
    /** PlannedMeal fuer ein Datum (oder null). */
    public PlannedMeal getPlannedMealForDate(LocalDate date) {
        if (plannedMeals == null) return null;
        for (PlannedMeal pm : plannedMeals) {
            if (pm.date().equals(date)) return pm;
        }
        return null;
    }

    /** Markiert das PlannedMeal fuer ein Datum als completed. */
    public void completePlannedMeal(LocalDate date, int actualServings) {
        if (plannedMeals == null) return;
        for (int i = 0; i < plannedMeals.size(); i++) {
            if (plannedMeals.get(i).date().equals(date) && !plannedMeals.get(i).completed()) {
                plannedMeals.set(i, plannedMeals.get(i).withCompleted(actualServings));
                return;
            }
        }
    }
