...
public final class TodayTimelineTest {
    ...
    @Test
    public void choosesLatestFinishedCalendarOccurrenceAsPredecessorRegardlessOfInputOrder() {
        LocalDateTime start = NOW.plusHours(2);
        PlanAssignment task = assignment("after-calendar", "Nachbereitung",
                start, start.plusMinutes(30));
        BusyInterval early = new BusyInterval("early", NOW, NOW.plusMinutes(30), "Früh",
                BusyInterval.TitleVisibility.VISIBLE);
        BusyInterval closest = new BusyInterval("closest", NOW.plusMinutes(10),
                start.minusMinutes(5), "Direkt davor", BusyInterval.TitleVisibility.VISIBLE);
        BusyInterval sameEndEarlierStart = new BusyInterval("same-end", NOW.plusMinutes(5),
                start.minusMinutes(5), "Gleicher Schluss", BusyInterval.TitleVisibility.VISIBLE);

        TodayTimeline timeline = TodayTimeline.from(dashboard(List.of(task),
                List.of(closest, early, sameEndEarlierStart)), NOW);
        TodayTimeline.Assignment entry = (TodayTimeline.Assignment) timeline.entries().stream()
                .filter(value -> value instanceof TodayTimeline.Assignment).findFirst().orElseThrow();

        assertSame(closest, entry.precedingCalendar());
    }

    @Test
    public void deduplicatesStableRowsAndRefreshesAtTheNextStartBoundary() {
        PlanAssignment later = assignment("duplicate", "Später",
                NOW.plusHours(2), NOW.plusHours(3));
        BusyInterval meeting = new BusyInterval("meeting", NOW.plusHours(1),
                NOW.plusHours(1).plusMinutes(15), "Termin",
                BusyInterval.TitleVisibility.VISIBLE);

        TodayTimeline timeline = TodayTimeline.from(dashboard(
                List.of(later, later), List.of(meeting, meeting)), NOW);

        assertEquals(2, timeline.entries().size());
        assertEquals(meeting.start(), timeline.nextRefreshAt());
        assertEquals(List.of("calendar:meeting", laterStableId(later)),
                timeline.entries().stream().map(TodayTimeline.Entry::stableId).toList());
    }

    private static String laterStableId(PlanAssignment assignment) {
        return "focus:" + assignment.workItem().id() + ":" + assignment.occurrenceKey();
    }
