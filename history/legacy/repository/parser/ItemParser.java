...
public class ItemParser {
    ...
    public static TrackedItem fromRow(Map<String, Object> row) {
        ...
        String plannedMealsStr = (String) typed.get("planned_meals");
        if (plannedMealsStr != null && !plannedMealsStr.isEmpty()) {
            item.plannedMeals = parsePlannedMeals(plannedMealsStr);
        }
    ...
    public static void toRow(SQLiteDatabase db, TrackedItem item) {
        ...
        if (item.plannedMeals != null && !item.plannedMeals.isEmpty()) {
            cv.put("planned_meals", item.plannedMeals.stream()
                .map(pm -> pm.date().toString() + ";" + pm.recipeId() + ";" + pm.servings()
                    + ";" + (pm.completed() ? 1 : 0) + ";" + pm.actualServings())
                .collect(Collectors.joining(",")));
        } else {
            cv.putNull("planned_meals");
        }
    ...
    /** Parst "date;recipeId;servings;completed;actualServings,..." String zu PlannedMeal-Liste. */
    private static java.util.List<TrackedItem.PlannedMeal> parsePlannedMeals(String raw) {
        java.util.List<TrackedItem.PlannedMeal> meals = new java.util.ArrayList<>();
        for (String entry : raw.split(",")) {
            String[] parts = entry.trim().split(";");
            if (parts.length != 5) continue;
            java.time.LocalDate date = ParseUtils.safeLocalDate(parts[0]);
            if (date == null) continue;
            Long recipeId = ParseUtils.safeLong(parts[1]);
            if (recipeId == null) continue;
            int servings = ParseUtils.safeInt(parts[2], 1);
            boolean completed = "1".equals(parts[3]);
            int actualServings = ParseUtils.safeInt(parts[4], 0);
            meals.add(new TrackedItem.PlannedMeal(date, recipeId, servings, completed, actualServings));
        }
        return meals;
    }
