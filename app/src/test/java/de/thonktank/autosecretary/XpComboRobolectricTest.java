...
public final class XpComboRobolectricTest {
    ...
    @Test public void stepAndRoutineUseTheirOwnFactorsAndTodayUndoIsExact() {
        ...
        RewardReceipt stepReceipt = new ToggleStep(repository, repository, repository, repository, clock).execute(step.id);
        ...
        RewardReceipt harvest = new HarvestOccurrence(repository, repository, repository, repository, clock).execute(occurrence.id);
        ...
        RewardReceipt undo = new UndoOccurrence(repository, repository, repository, repository, clock).execute(occurrence.id);
        ...
        RewardReceipt stepUndo = new ToggleStep(repository, repository, repository, repository, clock).execute(step.id);
        ...
        RewardReceipt recompleted = new ToggleStep(repository, repository, repository, repository, clock).execute(step.id);
        ...
        assertEquals(25, new de.thonktank.autosecretary.domain.usecase.LoadDashboard(
                new RoomTaskRepository(database), new RoomTaskRepository(database))
    ...
    @Test public void restCompletionOnlyFillsBeforeSeparateHarvest() {
        ...
        RewardReceipt fill = new CompleteRemainingSteps(repository, repository, repository, repository, clock).execute(occurrence.id);
        ...
        new HarvestOccurrence(repository, repository, repository, repository, clock).execute(occurrence.id);
    ...
    @Test public void widgetStyleCompletionFillsAndHarvestsAtomically() {
        ...
        RewardReceipt receipt = new CompleteOccurrence(repository, repository, repository, repository, clock).execute(occurrence.id);
    ...
    @Test public void lateRoutineStepEarnsTheSameComboGain() {
        ...
        RewardReceipt receipt = new ToggleStep(repository, repository, repository, repository, clock).execute(step.id);
    ...
    @Test public void repetitionXpTracksActualRatioAndCorrectionsAgainstFrozenPlan() {
        ...
        CorrectRepetitionResult correct = new CorrectRepetitionResult(repository, repository, repository, repository, clock);
    ...
    @Test public void correctionKeepsTheComboPolicyOfTheFirstPositiveBooking() {
        ...
        assertEquals(5, new RecordRepetitionResult(repository, repository, repository, repository, clock, source)
                .execute(step.id, 5).xp);
        selected[0] = new ComboPolicy(4, 1, ComboDecayTrigger.DAILY_OVERDUE);
        assertEquals(1, new CorrectRepetitionResult(repository, repository, repository, repository, clock, source)
    ...
    @Test public void finishForTodayPreservesPartialActualsAndRestFillsOnlyMissingSlots() {
        ...
        Occurrence occurrence = repository.openOccurrences().get(0);
        OccurrenceStep step = repository.occurrenceSteps(occurrence.id).get(0);
        new RecordRepetitionResult(repository, repository, repository, repository, clock).execute(step.id, 5);

        new FinishStepForToday(repository, repository, repository, repository, clock, ComboPolicySource.defaults()).execute(step.id);
    ...
    @Test public void nextDayAutoHarvestsPositivePartialWorkExactlyOnce() {
        ...
        clock.date = today;
        SettlePreviousPartialOccurrences settle = new SettlePreviousPartialOccurrences(repository, repository, repository, repository, repository, clock, ComboPolicySource.defaults());
