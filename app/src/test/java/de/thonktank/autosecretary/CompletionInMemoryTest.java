...
public final class CompletionInMemoryTest {
    ...
    @Test public void completionUndoAndRecompletionUseExactImmutableBookingsAndSchedule() {
        ...
        RewardReceipt completed = new CompleteOccurrence(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(occurrence.id);
        ...
        RewardReceipt undone = new UndoOccurrence(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(occurrence.id);
        ...
        RewardReceipt recompleted = new CompleteOccurrence(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(occurrence.id);
    ...
    @Test public void stepUndoKeepsOtherRewardsAndCanBeAppliedOnlyOnce() {
        ...
        RewardReceipt earned = new ToggleStep(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(step.id);
        assertEquals(10, earned.xp);
        assertTrue(repository.findOccurrenceStep(step.id).done);

        RewardReceipt reversed = new ToggleStep(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(step.id);
        ...
        assertEquals(10, new ToggleStep(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(step.id).xp);
    ...
    @Test public void arbitraryTodayConditionUndoReopensTheConditionAndProjection() {
        ...
        RewardReceipt closed = new CloseOngoingTask(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(task.id);
    ...
    @Test public void transactionRollbackRestoresEveryProjectionAndLedgerEntry() {
        ...
        assertThrows(IllegalStateException.class, () -> repository.transactions.inTransaction(() -> {
            new ToggleStep(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(step.id);
            repository.setXp(99);
    ...
    @Test public void rolloverCreatesTodaysInstanceWithUnfinishedAndFreshStepsOnce() {
        ...
        new ToggleStep(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(finished.id);

        clock.set(TODAY.plusDays(1));
        new MaterializeDueOccurrences(repository.catalog, repository.steps, repository.today, repository.flows, repository.transactions, clock, this::nextId).execute();
    ...
    @Test public void partialHarvestClosesOccurrenceAndCarriesOpenStepNextDay() {
        ...
        assertEquals(10, new HarvestOccurrence(repository.catalog, repository.steps, repository.today, repository.transactions, clock).execute(yesterday.id).xp);
    ...
    public void dashboardRemainsDeterministicUnderManyTasksStepsCombosAndHistory() {
        ...
        Dashboard dashboard = new LoadDashboard(repository.catalog, repository.steps, repository.today, repository.flows).execute(TODAY);
    ...
    private void create(String title, TaskSlot slot, Recurrence recurrence,
        ...
        if (!ongoing) {
            new CreateTask(repository.catalog, repository.steps, repository.today, repository.transactions, clock, this::nextId).execute(
