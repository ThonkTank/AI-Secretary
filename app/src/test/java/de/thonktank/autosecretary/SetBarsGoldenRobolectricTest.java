...
public final class SetBarsGoldenRobolectricTest {
    @Test public void allRequiredSetCountsKeepTheScrollableSetBars() throws Exception {
    ...
    @Test public void selectedEarlierSetKeepsTheCorrectionRing() throws Exception {
        verify(3, 412, 1f, DayPalette.Mode.LIGHT, 0,
                "set-bars-selected-3-412dp-10-light");
    }
    ...
    private static void verify(int sets, int widthDp, float fontScale,
        ...
        String name = explicitName == null ? "set-bars-" + sets + '-' + widthDp + "dp-"
                + Math.round(fontScale * 10) + '-' + mode.name().toLowerCase() : explicitName;
        GoldenAssertions.compare(SetBarsGoldenRobolectricTest.class,
